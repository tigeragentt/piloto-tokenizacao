...
/**
 * @title TObserver
 * @notice Remix/testnet version of Observer (v1.6.0).
 *         Deployment order:
 *           1. Deploy TObserverFunds — no args
 *           2. Deploy TObserver(_fund: TObserverFunds address)
 *
 *         Differences from production Observer.sol:
 *           - Uses simulation forwarder by default
 *           - reportSettlement() and reportAction() are public (no onlyOwner)
 *           - Depends on IObserverFunds interface, not the concrete contract
 *
...
contract TObserver is ReceiverTemplate {
    ...
    /// @param _fund Address of the deployed TObserverFunds contract.
    constructor(address _fund) ReceiverTemplate(SIMULATION_FORWARDER) {
        fund = IObserverFunds(_fund);
    }
    ...
    function reportAction(
        ...
        emit ActionReported(recordId, fundId, txHash, network, action, from, to, amount, block.timestamp);
    }
    ...
    function _reportSettlement(SettlementInput memory s) internal returns (uint256 recordId) {
        ...
        emit SettlementReported(
            recordId, s.fundId, s.orderId, s.intentHash, s.progress,
